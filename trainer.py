...
from model import EmoClassificationModel
from utils import labels, TokenizersCollateFn, EarlyStopper, EmoDataset, Mish


class Trainer():
    ''' Class trainer implemented with steps for model training/finetuning task.
      The new model uses a roberta base model and uses the emotion classifier
    ...
    def __init__(self, train_path, val_path, test_path, location, batch_size=20, epochs=10, lr=1.5E-04, second_ft=False):
        ...
        # load the model - roberta base model with emotion classifier
    ...
    def fit(self):
        ...
        # log each epoch
        logging.info(f"Training started at {datetime.datetime.now()}, optimizer: {self.optimizer}, scheduler: {self.scheduler}, learning rate: {self.lr}, loss function: {self.loss}")
    ...
    def evaluate(self):
        ...
        #         inputs, labels = batch
        #         inputs = (inputs[0].to(self.device), inputs[1].to(self.device))  # Move inputs to device
    ...
    #     ''' 
    #     X, y = batch
    #     X = (X[0].to(self.device), X[1].to(self.device))  # Move inputs to device
    #     labels = labels.to(self.device)  # Move labels to device
